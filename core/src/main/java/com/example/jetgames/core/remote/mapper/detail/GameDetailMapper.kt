...
class GameDetailMapper @Inject constructor(
    ...
    override fun mapFromModel(model: GameDetailsDto?): GameDetails {
        return GameDetails(
            ...
            metacritic_platforms = metacriticPlatformMapper.mapModelList(model?.metacritic_platforms),
            name = model?.name,
            parent_platforms = parentPlatformMapper.mapModelList(model?.parent_platforms),
            rating = model?.rating,
